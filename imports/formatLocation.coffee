...
formatLocation = ({name, admin2Name, admin1Name, countryName, featureCode}, useHTML=false) ->
  result = _.chain([name, admin2Name, admin1Name, countryName])
    .compact()
    .uniq()
    .value()
    .join(", ")
  if useHTML
    return result + """ <span
      class="feature-code"
      data-toggle="tooltip"
      title="#{featureCodes[featureCode]}">[#{featureCode}]</span>"""
  else
